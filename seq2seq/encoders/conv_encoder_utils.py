...
def conv_encoder_stack(inputs, nhids_list, kwidths_list, dropout_dict, mode):
  ...
  for layer_idx in range(len(nhids_list)):
    ...
    if nin != nout:
      #mapping for res add
      res_inputs = linear_mapping(next_layer, nout, dropout=dropout_dict['src'], var_scope_name="linear_mapping_cnn_" + str(layer_idx))    
    else:
...
def conv_decoder_stack(target_embed, enc_output, inputs, nhids_list, kwidths_list, dropout_dict, mode):
  ...
  for layer_idx in range(len(nhids_list)):
    ...
    if nin != nout:
      #mapping for res add
      res_inputs = linear_mapping(next_layer, nout, dropout=dropout_dict['hid'], var_scope_name="linear_mapping_cnn_" + str(layer_idx))      
    else:
      res_inputs = next_layer
    #dropout before input to conv
    next_layer = tf.contrib.layers.dropout(
...
def make_attention(target_embed, encoder_output, decoder_hidden, layer_idx):
  with tf.variable_scope("attention_layer_" + str(layer_idx)):
    ...
    dec_hidden_proj = linear_mapping(decoder_hidden, embed_size, var_scope_name="linear_mapping_att_query")  # M*N1*k1 --> M*N1*k
    ...
    att_out = linear_mapping(att_out, decoder_hidden.get_shape().as_list()[-1], var_scope_name="linear_mapping_att_out")
  return att_out
