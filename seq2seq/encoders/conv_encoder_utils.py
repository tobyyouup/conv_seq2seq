...
class ConvEncoderUtils():
  ...
  #@staticmethod
  def linear_mapping_stupid(self, inputs, out_dim, in_dim=None, dropout=1.0, var_scope_name="linear_mapping"):
  ...
  def linear_mapping(self, inputs, out_dim, in_dim=None, dropout=1.0, var_scope_name="linear_mapping"):
    with tf.variable_scope(var_scope_name):
      input_shape = inputs.get_shape().as_list()    # static shape. may has None
  ...
  #@staticmethod
  def conv_encoder_stack(self, inputs, nhids_list, kwidths_list, dropout_dict, mode):
    next_layer = inputs
    for layer_idx in range(len(nhids_list)):
      ...
      if nin != nout:
        #mapping for res add
        res_inputs = self.linear_mapping(next_layer, nout, dropout=dropout_dict['src'], var_scope_name="linear_mapping_cnn_" + str(layer_idx))    
      else:
  ...
  #@staticmethod
  def conv_decoder_stack(self, target_embed, enc_output, inputs, nhids_list, kwidths_list, dropout_dict, mode):
    next_layer = inputs
    for layer_idx in range(len(nhids_list)):
      nin = nhids_list[layer_idx] if layer_idx == 0 else nhids_list[layer_idx-1]
      nout = nhids_list[layer_idx]
      if nin != nout:
        #mapping for res add
        res_inputs = self.linear_mapping(next_layer, nout, dropout=dropout_dict['hid'], var_scope_name="linear_mapping_cnn_" + str(layer_idx))      
      else:
        res_inputs = next_layer
      #dropout before input to conv
      next_layer = tf.contrib.layers.dropout(
      ...
      att_out = self.make_attention(target_embed, enc_output, next_layer, layer_idx) 
